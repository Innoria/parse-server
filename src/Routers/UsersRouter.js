...
export class UsersRouter extends ClassesRouter {
    ...
    handleMe(req) {
        ...
        return rest.find(req.config, Auth.master(req.config), '_Session', { sessionToken }, { include: 'user' }, req.info.clientSDK)
            .then((response) => {
                if (!response.results ||
    ...
    handleLogIn(req) {
        ...
        if (!username && !email && !phoneNumber) {
            throw new Parse.Error(Parse.Error.USERNAME_MISSING, 'username/email/phoneNumber is required.');
        }
        if (!password) {
    ...
    handleLogOut(req) {
        ...
        if (req.info && req.info.sessionToken) {
            return rest.find(req.config, Auth.master(req.config), '_Session', { sessionToken: req.info.sessionToken }, undefined, req.info.clientSDK).then((records) => {
