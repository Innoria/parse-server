...
class UsersRouter extends _ClassesRouter2.default {
    ...
    handleLogIn(req) {
        ...
        if (!payload.username && req.query.username || !payload.email && req.query.email || !payload.phoneNumber && req.query.phoneNumber) {
            payload = req.query;
        }
        const {
            username,
        ...
        if (!username && !email) {
            throw new _node2.default.Error(_node2.default.Error.USERNAME_MISSING, 'username/email/phoneNumber is required.');
        }
        if (!password) {
            throw new _node2.default.Error(_node2.default.Error.PASSWORD_MISSING, 'password is required.');
        }
        if (typeof password !== 'string' || email && typeof email !== 'string' || username && typeof username !== 'string' || phoneNumber && typeof phoneNumber !== 'string') {
            throw new _node2.default.Error(_node2.default.Error.OBJECT_NOT_FOUND, 'Invalid username/password/phoneNumber.');
        }
        ...
        if (email && username && phoneNumber) {
            query = { email, username, phoneNumber };
        } else if (email) {
            query = { email };
        } else if (phoneNumber) {
            query = { phoneNumber };
        } else {
            query = { $or: [{ username }, { email: username }, { phoneNumber: username }] };
        }
